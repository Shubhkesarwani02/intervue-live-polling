...
function initializeSocketIO(res: any) {
  ...
  try {
    ...
    io.on("connection", (socket) => {
      ...
      // Student joins
      socket.on("join-as-student", (studentName: string) => {
      ...
      socket.on("ask-question", (questionData: Omit<Question, "id" | "startTime">) => {
        currentQuestion = {
          ...questionData,
      ...
      socket.on("submit-answer", (answerData: { answer: string }) => {
        if (!currentQuestion) return
